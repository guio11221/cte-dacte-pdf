...
function normalizeEventRoot(raw) {
...
function getTomador(infCte) {
    const toma3 = infCte?.ide?.toma3;
    const toma4 = infCte?.ide?.toma4;
    if (toma4)
        return readParty(toma4);
    const code = firstText(toma3?.toma);
    if (code === '0')
        return readParty(infCte?.rem);
    if (code === '1')
        return readParty(infCte?.exped);
    if (code === '2')
        return readParty(infCte?.receb);
    if (code === '3')
        return readParty(infCte?.dest);
    return undefined;
}
function getTipoCte(value) {
    const map = {
        '0': 'Normal',
        '1': 'Complementar',
        '2': 'Anulacao',
...
function getFormaEmissao(value) {
    const map = {
        '1': 'Normal',
        '4': 'EPEC',
        '5': 'FS-DA',
...
function parseComponentes(vPrest) {
    return arr(vPrest?.Comp).map((item) => ({
        label: firstText(item.xNome),
        value: formatMoney(item.vComp)
    }));
}
function parseImpostos(imp) {
    const icms = imp?.ICMS ?? {};
...
function parseRodoviario(infCteNorm) {
...
function parseObservacoes(infCte) {
    const compl = infCte?.compl ?? {};
    const obs = [];
    if (compl.xCaracAd)
        obs.push(`Caracteristica adicional: ${compl.xCaracAd}`);
    if (compl.xCaracSer)
        obs.push(`Caracteristica servico: ${compl.xCaracSer}`);
    if (compl.xEmi)
        obs.push(`Emissor: ${compl.xEmi}`);
    if (compl.xObs)
        obs.push(String(compl.xObs));
    for (const item of arr(compl.ObsCont)) {
        const campo = firstText(item['@_xCampo']);
        const texto = firstText(item.xTexto);
        if (campo || texto)
            obs.push(`${campo ? `${campo}: ` : ''}${texto}`);
    }
...
function parseEventEmitente(infEvento) {
    return {
...
export function parseCteEventXml(xml) {
    const raw = parser.parse(xml);
...
export function parseCteDocumentXml(xml) {
    const raw = parser.parse(xml);
...
//# sourceMappingURL=cte-parser.js.map